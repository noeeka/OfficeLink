...
class SCPClient(object):
    ...
    def put(self, files, remote_path=b'.',
            recursive=True, preserve_times=False):
        """
    ...
    def _send_files(self, files):
        for name in files:
            ...
            self._recv_confirm()
            file_pos = 0
            basename_a = basename.decode().replace('\\', '/')
            if self._progress:
                if size == 0:
                    # avoid divide-by-zero
                    self._progress(basename_a, 1, 1)
                else:
                    self._progress(basename_a, size, 0)
            ...
            while file_pos < size:
                ...
                if self._progress:
                    self._progress(basename_a, size, file_pos)
            chan.sendall('\x00')
            file_hdl.close()
            self._recv_confirm()
    ...
    def _recv_file(self, cmd):
        ...
        try:
            ...
            chan.close()
            raise
        path_a = path.decode().replace('\\', '/')
        if self._progress:
            if size == 0:
                # avoid divide-by-zero
                self._progress(path_a, 1, 1)
            else:
                self._progress(path_a, size, 0)
        buff_size = self.buff_size
        pos = 0
        chan.send(b'\x00')
        try:
            while pos < size:
                ...
                pos = file_hdl.tell()
                if self._progress:
                    self._progress(path_a, size, pos)
