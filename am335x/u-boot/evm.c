...
#define RMII_MODE_ENABLE	(0x5 | (3<<6))
#define MII_MODE_ENABLE		0x0
#define RMII_RGMII_MODE_ENABLE (0x9 | 1<<6)
...
#define RTL8211CL_PHYIDR1    0x1c
#define RTL8211CL_PHYIDR2    0xc912
...
extern int azm335x_board_init(void);
...
void spl_board_init(void)
{
    ...
    {
        printf("probe tps65217 failure\n");
    }
    else
    {
        azm335x_board_init();
    }
	printf("\nU-Boot SPL %s (%s - %s)\n", &((char*)U_BOOT_VERSION)[7], U_BOOT_DATE,
		U_BOOT_TIME);
    /* Set MPU Frequency to 720MHz */
    printf("Set MPU Frequency to 720MHz ...");
    mpu_pll_config(MPUPLL_M_720);
    printf("done\n");
}
#endif

static int full_dram_test(unsigned long* base)
{
    unsigned long i;
    //printf("full_dram_test fill\r\n");
    for(i=0;i<(PHYS_DRAM_1_SIZE/4);i+=0x100)
    {
        base[i] = i;
#if 0
        if(((i>>8)&(0x1000-1))==0)//notify pre 4M
        {
            printf(".");
        }
#endif
    }
    //printf("\r\nfull_dram_test compare\r\n");
    for(i=0;i<(PHYS_DRAM_1_SIZE/4);i+=0x100)
    {
        if(i != base[i])
        {
            printf("mem err base[%d] = 0x%08x\r\n", i, base[i]);
        }
#if 0
        if(((i>>8)&(0x1000-1))==0)//notify pre 4M
        {
...
{
    unsigned long *pserial = (unsigned long*) (CONFIG_SPL_TEXT_BASE);    
    serialnr->high = pserial[0];
    serialnr->low = pserial[1];
}
...
int board_init(void)
{
    ...
    board_id = AZM335X_BOARD;
    configure_evm_pin_mux(board_id, 1, 1);
...
static void evm_phy_init(char *name, int addr)
{
    ...
    while(phy_retry--)
    {
        ...
        {
            printf("miiphy_read %d failed, %x:%x\n", addr, phyid1, phyid2);
            for(i=0;i<32;i++)
            {
                phyid1 =0;
                miiphy_read(name, i, MII_PHYSID1, &phyid1);
                printf(" %d %x\n", i, phyid1);
            }
            //udelay(1000000);
            continue;
    ...
    printf("found phy at addr %d, phyid %x:%x\n", addr, phyid1, phyid2);
    ...
    if ((phyid1 == RTL8211CL_PHYIDR1) && (phyid2 == RTL8211CL_PHYIDR2)) {
        miiphy_write(name, addr, MII_BMCR, BMCR_RESET);
        miiphy_write(name, addr, MII_BMCR, BMCR_SPEED100|BMCR_FULLDPLX);
		printf("Force to 100BaseT FULL\n");
		return;
    }
